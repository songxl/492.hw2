...
void astar(string puzzle) {
	...
	root_state->G = 0;
	root_state->H = computingH1(root_state->puzzle);
...
if(test_cout)		cout << "\nThe top is: "<< frontier.top()->puzzle << endl;
...
if(test_cout)				cout << "find the root"<<endl;
if(test_cout)				cout << "depth is "<< solution_depth<<endl;
				//find the target, return the depth, queue size etc..
				return;
			}						//find the solution
if(test_cout)		cout << "Pop out the first puzzle in pq: "<< front->puzzle << endl;
if(test_cout)		cout << "It's F = "<< front->G + front->H << endl;
...
if(test_cout)		cout << "There is a better way of G= "<< new_G << endl;
...
if(test_cout)		cout << "This state already on open_list"<< next_state << endl;
				...
				new_state->H = computingH1(next_state);
...
if(test_cout)	cout << "NEW STATE: push the state " << next_state << " to frontier\n";
if(test_cout)		cout << "After push, The front is: "<< frontier.top()->puzzle << endl;
if(test_cout)		cout << "The top F: "<< frontier.top()->H + frontier.top()->G << endl;
if(test_cout)		cout << "The length of the frontier is: "<< frontier.size() << endl <<endl;
...
int main() {
    ...
    while(next_puzzle.size() ) {
	    puzzle_counter++;
if(test_cout)			cout << "\n\nNO. " << puzzle_counter<<endl;

        astar(next_puzzle);
if(test_cout)			cout << "solution_depth " << solution_depth <<endl;
    ...
    for (int i = 0; i <10; i ++){
//    	cout << "sum_solution_depth["<<i<<"] is "<<sum_solution_depth[i]<<endl;
//    	cout << "number_in_CG["<<i<<"] is "<<number_in_CG[i]<<endl;
		...
		cout << setw(9) << i+1 << setw(15) << avg_heuristic[i]<< "   |" << setw(15) << avg_solution_depth[i] <<"   |" << setw(15) << avg_goals_tested[i] << "   |" << setw(15) << avg_max_frontier_size[i] <<endl;
		}
}
