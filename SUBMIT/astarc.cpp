...
void astar(string puzzle) {
	...
	root_state->G = 0;
	root_state->H = 5 * computingH1(root_state->puzzle);
	...
	while (!frontier.empty()){			//do until no frontier
		...
		for (int direction = UP; direction <= LEFT; direction ++){
			...
			if (open_list.find(next_state) != open_list.end()) {		//already on open_list, check G
				...
				new_state->H = 5 * computingH1(next_state);
...
int main() {
    ...
    for (int i = 0; i <10; i ++){
		...
		cout << setw(9) << i+1  <<"    |" << setw(13) << avg_heuristic[i]<< "   |" << setw(15) << avg_solution_depth[i] <<"   |" << setw(15) << avg_goals_tested[i] << "   |" << setw(15) << avg_max_frontier_size[i] <<endl;
		}
}
