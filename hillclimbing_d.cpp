...
void hillclimbing(string puzzle) {
	...
	while (!out_of_choice){			//do until no choice or solved the puzzle
		...
		for (int direction = UP; direction <= LEFT; direction ++){
			...
			if (computingH2(next_state) < second_minimum){			//find a local minimum
				second_choice_state = next_state;
				second_minimum = computingH2(next_state);
			}
			...
			else if (step_counter <= step_limit){
				//use the second choice
				current_state->puzzle = second_choice_state;
				current_state->H = computingH2(second_choice_state);
				current_state->depth ++;
			}
			else
				out_of_choice = 1;
...
//    	cout << "sum puzzle solved i " << sum_puzzle_solved[i]<<endl;
