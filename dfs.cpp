...
void dfs(string puzzle) {
	...
	while (!frontier.empty()){			//do until no frontier
		...
		if(test_cout) cout << " is it explored? " <<front->explored <<endl;
		if (front->explored == 1){
if(test_cout)		cout << "The puzzle " << front->puzzle << " has been explored, pop it\n";
			frontier.pop_front();
			continue;
		}
if(test_cout)		cout << "exploring the puzzle "<< frontier.front().puzzle << endl;
		for (int direction = UP; direction <= LEFT; direction ++){
			...
			if (next_state == "") {
				...
				if(test_cout)		cout <<"mark " << front->puzzle<< " as explored"<<endl;
...
if(test_cout)				cout << "depth is "<< solution_depth<<endl;
				//cout << "frontier_max_size " << frontier_max_size  <<endl;
				return;
			}						//find the solution
			else if (explored_sets.find(next_state) == explored_sets.end()){ //has not visited yet
if(test_cout)				cout << "push the state " << next_state << " to queue\n";
if(test_cout)		cout <<"mark " << front->puzzle<< " as NOT explored"<<endl;
				number_goals_tested ++;			//whenver examine isGoal, increment
			...
			if(test_cout)		cout <<"Child explored. mark " << front->puzzle<< " as explored"<<endl;
			front->explored = 1;
			}
		}
	}	
...
int main() {
    ...
    while(next_puzzle.size()) {
	    puzzle_counter++;
		cout << "NO. " << puzzle_counter<<endl;

        dfs(next_puzzle);
		cout << "solution_depth " << solution_depth <<endl;
...
//    	cout << "number_in_CG["<<i<<"] is "<<number_in_CG[i]<<endl;
		...
		cout << setw(9) << i+1 << "   |" << setw(15) << avg_solution_depth[i] <<"   |" << setw(15) << avg_goals_tested[i] << "   |" << setw(15) << avg_max_frontier_size[i] <<endl;
		}
}
